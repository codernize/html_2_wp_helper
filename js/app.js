    ...
    $('.slickSlider').each(function (index, el) {
        ...
        if ($el.attr('data-options')) {
            $el.attr('data-options').split(';').forEach(function (option, _index) {
                ...
                if (opt[0]) opts[opt[0]] = parseValue(opt.slice(1).join(':'));
            });
        }
        ...
        Foundation.onImagesLoaded($images, function () {
            slickSliderChild.index = $el.slick(opts);

        });

        // var options = new Foundation.Accordion($('#some-accordion'), options);
        // 
    });


    $('button[data-toggle="responsive-menu"]').on('click', function (event) {
